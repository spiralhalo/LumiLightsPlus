...
#include lumi:shaders/data/shading_data.glsl
 ...
 *******************************************************/
	...
	float fastLight(vec2 light, vec3 normal) {
		...
		return 0.2 + 0.8 * result;
			...
			float denoisedShadowFactor(
				...
				#ifdef TAA_ENABLED
				{
					...
					vec2 uvJitter	   = taaJitter(inv_size, frx_renderFrames);
		...
		vec3 reflectRough(sampler2DArray resources, vec3 toFrag, vec3 normal, float materialx, vec2 screenCoord, vec2 screenSize, out vec3 jitterPrc)
		{
			const float strength = 0.6;
			vec3 jitterRaw = getRandomVec(resources, screenCoord, screenSize) * 2.0 - 1.0;
			jitterPrc = jitterRaw * strength * materialx * materialx;
			return normalize(reflect(toFrag, normal) + jitterPrc);
		}

		vec3 reflectRough(sampler2DArray resources, vec3 toFrag, vec3 normal, float materialx, vec2 screenCoord, vec2 screenSize)
		{
			vec3 ignored;
			return reflectRough(resources, toFrag, normal, materialx, screenCoord, screenSize, ignored);
		}
	...
	#endif // not VERTEX_SHADER
#endif // POST_SHADER or FORWARD_TRANSLUCENT
