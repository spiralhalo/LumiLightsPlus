...
vec3 skyBase(vec3 toSky, vec3 fallback) {
	vec3 result = vec3(0.0);
	if (frx_worldIsOverworld == 1) {
		#if SKY_MODE == SKY_MODE_LUMI
		result  = atmosv_SkyRadiance;
		#else
		float mul = 1.0 + frx_worldIsMoonlit * frx_skyLightTransitionFactor;
		vec3 fallback1 = hdr_fromGamma(fallback) * mul;
		result = fog(vec4(fallback1, 1.0), frx_viewDistance * 4.0, toSky, false).rgb;
		#endif

		result += vec3(frx_skyFlashStrength * LIGHTNING_FLASH_STR);

		float skyGradient = pow(l2_clampScale(0.625 + v_cameraAt, -0.125 + v_cameraAt, toSky.y), 3.0);
		result = mix(result.rgb, fogColor(false, toSky), skyGradient);
	}
	...
	if (frx_worldIsEnd == 1){
		...
		result += 0.08 * atmosv_FogRadiance * g;// * sup;
		// result.rgb += 0.05 * atmosv_FogRadiance * h;
	
		result *= l2_clampScale(0.0, l2_clampScale(128.0, 10.0, frx_cameraPos.y), toSky.y * 0.5 + 0.5);
	}

	if (frx_worldIsOverworld + frx_worldIsEnd < 1) {
		result = hdr_fromGamma(fallback) * (1.0 + float(frx_worldIsEnd) * 1.0);
	}
	
	if (frx_worldIsNether == 1) {
		result = atmosv_FogRadiance;
	}

	return result;
}

vec4 voidCore(vec3 result, vec3 toSky) {
	if (frx_worldIsOverworld == 1) {
		// VOID CORE
		...
		result = mix(voidColor, result, v_not_in_void);
	}

	return vec4(result, 1.0);
}

vec4 basicSky(vec3 toSky, vec3 result) {
	if (frx_cameraInWater == 1) {
		result = atmosv_WaterFogRadiance;
	}
...
vec4 customSky(vec3 result, sampler2DArray resources, vec3 toSky, vec3 fallback, bool isUnderwater, float skyVisible, float celestVisible) {
	...
	if (frx_worldIsOverworld == 1 && !isUnderwater) {
		...
		result += celestColor.rgb * (1. - frx_rainGradient) * celestVisible;
		#endif
	}
	...
	if (frx_worldIsOverworld + frx_worldIsEnd > 0 && !isUnderwater) {
		...
		result += starRadiance * skyVisible;
		#endif
	}
